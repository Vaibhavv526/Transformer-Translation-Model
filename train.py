...
def greedy_decode(model,source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    #precompute the encoder output and reuse it for every token we get free the decoder
...
def run_validation(model, validation_ds,tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer,num_examples =2):
    ...
    # size of the xontrol window (just use the default size)
    console_width = 80
...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        trainer = BpeTrainer(vocab_size=30000,special_tokens = ["[UNK]", "[PAD]", "[SOS]", "[EOS]"], min_frequency=2)
...
def get_ds(config):
    ...
    #keep 90% for traning and 10% for validation or testing 
 ...
 #tensorboard
    ...
    for epoch in range (initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            encoder_input = batch ['encoder_input'].to(device) ## 8 to seq_len
            decoder_input = batch['decoder_input'].to(device)  ## 8 to sql_len
