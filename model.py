...
class LayerNormalization (nn.Module):
    ...
    def forward(self, x):
            ...
            return self.alpha * (x - means)/ (std+ self.eps) + self.bias
...
class MultiheadAttentionBlock(nn.Module):
    ...
    def __init__(self,d_model: int, h: int, dropout: float)-> None:
        ...
        self.d_model = d_model
        self.h = h
        assert d_model % h == 0, "d_model us not divisible by h"
    ...
    def forward (self, q,k,v,mask):
        ...
        # (Batch, seq_len,d_model) --> (Batchm seq_len, d_model)
        return self.W_o(x)
...
# Now we can design Transformer According to our need Like our aim is to design it for English to hindi translation 
